...
export default function BusMediumModal({ isOpen, onClose, onSelectToBook }: BusMediumModalProps) {
  ...
  const busMediumCar = CARS.find(c => c.id === 'bus-medium') || CARS[2];

  const handleWhatsAppBooking = () => {
    const waNumber = '6281288748745';
    const message = `Halo Araka Trans Travel (PT. Generasi Baru Araka), saya berminat memesan/sewa unit *Bus Medium Jetbus 3* (Mulai Rp 2.000.000 / hari). Mohon info ketersediaan jam, tanggal & rincian sewa. Terima kasih!`;
  ...
  const getDisplayedImage = () => {
    if (activePhotoTab === 'interior_led') {
      return '/bus_medium_real_interior_led.jpg';
    }
    if (activePhotoTab === 'interior_seats') {
      return '/bus_medium_real_interior_seats.jpg';
    }
    return '/bus_medium_real_exterior.jpg';
  };

  const getDisplayedBadgeText = () => {
    if (activePhotoTab === 'interior_led') return 'Kabin LED Blue Light & Bantal Leher';
    if (activePhotoTab === 'interior_seats') return 'Full Kabin 33 Reclining Seats';
    return 'Jetbus 3 Pariwisata B 7143 WGA';
  };
        ...
        >
          ...
          <div className="p-4 sm:p-5 overflow-y-auto space-y-4 text-left">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-3">
              <div>
                ...
                <p className="text-xs font-bold text-slate-500 mt-0.5">
                  Bus Pariwisata & Event • Capacity 33-35 Kursi Penumpang
                </p>
              </div>
              ...
              <div className="bg-amber-500 text-white font-sans font-extrabold text-xs sm:text-sm px-3.5 py-1.5 rounded-xl shadow-sm w-fit shrink-0 flex items-center gap-1.5">
                ...
                <span>Mulai Dari Rp 2.000.000 / hari</span>
              </div>
            </div>
            ...
            <p className="text-xs sm:text-sm text-slate-600 leading-relaxed font-medium">
              Pilihan ideal untuk rombongan wisata, gathering, study tour & event kantor dengan armada Jetbus 3 Araka Trans 33 kursi reclining seats.
            </p>
          ...
          <div className="p-3.5 bg-slate-50 border-t border-slate-200 shrink-0 flex flex-col sm:flex-row items-center gap-2">
            ...
            >
              ...
              <span>Pesan Bus Medium via WA</span>
            </button>

            <button
              onClick={() => {
                onClose();
                onSelectToBook(busMediumCar);
              }}
