...
interface AlphardModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSelectToBook: (car: Car) => void;
}

export default function AlphardModal({ isOpen, onClose, onSelectToBook }: AlphardModalProps) {
  const [activePhotoTab, setActivePhotoTab] = useState<'exterior' | 'interior_seats' | 'interior_front'>('exterior');
  ...
  const alphardCar = CARS.find(c => c.id === 'toyota-alphard') || CARS[3];

  const handleWhatsAppBooking = () => {
    const waNumber = '6281288748745';
    const message = `Halo Araka Trans Travel (PT. Generasi Baru Araka), saya berminat memesan/sewa unit *Toyota Alphard VIP Class* (Mulai Rp 2.400.000 / hari). Mohon info ketersediaan jam, tanggal & rincian sewa. Terima kasih!`;
  ...
  const getDisplayedImage = () => {
    ...
    return '/alphard_real_exterior.jpg';
  };
        ...
        >
          ...
          <div className="bg-[#0c2340] text-white px-5 py-4 flex items-center justify-between border-b border-blue-900 shrink-0">
            <h3 className="font-display font-black text-sm sm:text-base text-white uppercase tracking-tight leading-tight">
              INFORMASI DETAIL TOYOTA ALPHARD
            </h3>
          ...
          <div className="p-4 sm:p-5 overflow-y-auto overscroll-contain space-y-4 text-left">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-3">
              <div>
                ...
                <p className="text-xs font-bold text-slate-500 mt-0.5">
                  VIP Luxury Class • Capacity 7 Kursi Penumpang VIP
                </p>
              </div>
            ...
            <p className="text-xs sm:text-sm text-slate-600 leading-relaxed font-medium">
              Pilihan kendaraan premium VIP dengan First-Class Executive Captain Seats, kabin senyap, susunan interior mewah, dan pelayanan supir profesional.
            </p>
