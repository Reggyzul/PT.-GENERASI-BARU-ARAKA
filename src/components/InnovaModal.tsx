...
interface InnovaModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSelectToBook: (car: Car) => void;
}

export default function InnovaModal({ isOpen, onClose, onSelectToBook }: InnovaModalProps) {
  ...
  const innovaCar = CARS.find(c => c.id === 'innova-reborn') || CARS[2];

  const handleWhatsAppBooking = () => {
    const waNumber = '6281288748745';
    const message = `Halo Araka Trans Travel (PT. Generasi Baru Araka), saya berminat memesan/sewa unit *Toyota Innova Reborn* (Mulai Rp 1.000.000 / hari). Mohon info ketersediaan jam, tanggal & rincian sewa. Terima kasih!`;
    window.open(`https://wa.me/${waNumber}?text=${encodeURIComponent(message)}`, '_blank', 'noreferrer');
  };

  const facilitiesList = [
    'Full AC Triple Blower',
    'Executive Reclining Leather Seats',
    'Hand Sanitizer',
    'Tissue',
  ...
  const getDisplayedImage = () => {
    if (activePhotoTab === 'interior_rear') {
      return '/innova_reborn_real_interior_rear.jpg';
    }
  ...
  const getDisplayedBadgeText = () => {
    if (activePhotoTab === 'interior_rear') return 'Kabin Belakang Leather Seats';
    if (activePhotoTab === 'interior_front') return 'Kabin Tengah Leather Seats';
    return 'Unit Black Metallic B 1707 WIV';
  };
        ...
        >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-amber-300 font-medium">
                  Tarif Mulai Rp 1.000.000 / Hari • Include Mobil, Supir & BBM
                </p>
              </div>
            </div>
          ...
          <div className="p-5 sm:p-7 space-y-6 max-h-[80vh] overflow-y-auto overscroll-contain">
            ...
            <div className="bg-blue-50/60 p-4 rounded-2xl border border-blue-100 space-y-2">
              <h4 className="font-display font-bold text-sm text-[#0c2340]">Deskripsi Armada Innova Reborn:</h4>
              <p className="text-xs text-slate-700 leading-relaxed font-medium">
                Toyota Innova Reborn merupakan kendaraan MPV eksekutif terfavorit untuk perjalanan dinas kantor, kunjungan kerja, penjemputan tamu VIP, maupun liburan keluarga. Menawarkan kenyamanan suspensi super halus, kabin senyap, dan interior kursi kulit berkualitas tinggi.
              </p>
            </div>
            ...
            <div className="bg-slate-50 p-4 sm:p-5 rounded-2xl border border-slate-200 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h4 className="font-display font-bold text-sm text-[#0c2340]">Spesifikasi Teknis Armada Innova Reborn:</h4>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                <div className="bg-white p-3 rounded-xl border border-slate-200 space-y-0.5">
                  ...
                  <p className="font-bold text-[#0c2340]">Mulai Rp 1.000.000 / hari</p>
                </div>
                <div className="bg-white p-3 rounded-xl border border-slate-200 space-y-0.5">
                  <span className="text-[10px] text-slate-400 font-bold uppercase">Kapasitas Penumpang</span>
                  <p className="font-bold text-[#0c2340]">7 Kursi Executive Leather Seats</p>
                </div>
                <div className="bg-white p-3 rounded-xl border border-slate-200 space-y-0.5">
                  <span className="text-[10px] text-slate-400 font-bold uppercase">Fasilitas Utama</span>
                  <p className="font-bold text-[#0c2340]">Full AC Triple Blower, Reclining Seats, Disinfektan</p>
                </div>
                <div className="bg-white p-3 rounded-xl border border-slate-200 space-y-0.5">
                  <span className="text-[10px] text-slate-400 font-bold uppercase">Kondisi Armada</span>
                  <p className="font-bold text-[#0c2340]">Black Metallic Unit B 1707 WIV</p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-slate-50 px-5 py-4 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3 shrink-0">
            <div className="text-center sm:text-left">
              ...
              <span className="font-display font-black text-lg text-[#0c2340]">
                Mulai Rp 1.000.000 <span className="text-xs font-normal text-slate-500">/ hari</span>
              </span>
            </div>
            ...
            <div className="flex items-center gap-2.5 w-full sm:w-auto">
              ...
              >
                ...
                <span>Pesan Innova Reborn via WA</span>
              </button>
            </div>
          </div>
